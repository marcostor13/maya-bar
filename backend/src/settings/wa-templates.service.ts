...
export class WaTemplatesService {
  ...
  async sync(tenantId: string): Promise<WaTemplate[]> {
    ...
    if (!token || !wabaId) {
      throw new BadRequestException('Configura el Access Token y el WABA ID en Cloud API primero');
    }
    const res = await fetch(
      `https://graph.facebook.com/v19.0/${wabaId}/message_templates?limit=100`,
      { headers: { Authorization: `Bearer ${token}` } },
    );
    if (!res.ok) throw new BadRequestException(`Meta API ${res.status}: ${await res.text()}`);
    ...
    for (const t of data.data ?? []) {
      ...
      const doc = await this.templateModel.findOneAndUpdate(
        ...
        {
          tenantId: tid, metaId: t.id, name: t.name,
          category: t.category as WaTemplate['category'],
          language: t.language,
          status: t.status as WaTemplate['status'],
          body, headerType: header?.format, headerText: header?.text, footer,
  ...
  async create(tenantId: string, dto: CreateWaTemplateDto): Promise<WaTemplate> {
    const config = await this.getConfig(tenantId);
    ...
    return this.templateModel.create({
      tenantId: tid, metaId: created.id, name: dto.name,
      category: dto.category, language: dto.language, status: 'PENDING',
      body: dto.body, headerText: dto.headerText, footer: dto.footer, rawComponents: components,
    });
  }
  ...
  async remove(tenantId: string, templateId: string): Promise<void> {
    ...
    if (!template || template.tenantId.toString() !== tenantId) throw new NotFoundException();
    const config = await this.getConfig(tenantId);
    if (config?.waAccessToken && template.metaId) {
      await fetch(
        ...
        { method: 'DELETE', headers: { Authorization: `Bearer ${config.waAccessToken}` } },
      ).catch(() => {});
    }
