...
export class AiAgentsController {
  ...
  addDoc(
    ...
    assertRole(req.user.role, CRM_ROLES);
    if (!dto.filename || !dto.url)
      throw new BadRequestException('Faltan filename o url');
  ...
  addFile(
    @Param('id') id: string,
    @Body() dto: AgentFileDto,
    @Request() req: AuthReq,
  ) {
    assertRole(req.user.role, CRM_ROLES);
    if (!dto.alias || !dto.url || !dto.filename)
      throw new BadRequestException('Faltan alias, filename o url');
